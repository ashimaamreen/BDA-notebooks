...
select 
         ord.order_num                                                          as ID 
        ,'52591'                                                                as campaign_code 
        ,'Late Payers BAU'                                                      as campaign_name 
        ...
        ,max(churn.churn_score)                                                 as customised_field2 
        ,value.cvm_value_score                                                  as customised_field3 
...
from  
    ...
    left join gms.cx_cvm_churn          as churn 
        on churn.asset_row_id = asset.row_id 
         
    left join gms.cx_cvm_value          as value 
    ...
    left outer join ( 
        ...
        WHERE 
                store.store_name = '47891_AVAYA_NRMAOTM' 
        ) as avayawash 
...
-- due date 21 days ago 
    and datediff(now(),from_utc_timestamp(ord.order_dt,'AEST')) = 21 
-- not low risk and low value 
    and (churn.churn_score > 14.2 
         or value.cvm_value_score > 12.29) 
-- not in HRHV 
